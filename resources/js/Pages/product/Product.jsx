...
/**
 * ✅ Product List with Variant-wise Stocks[] support (controller loads variants.stocks)
 * Controller:
 *  ->with(['category', 'brand', 'variants', 'variants.stocks'])
 *
 * FIXES:
 * ✅ total stock uses variant.stocks sum (not variant.stock)
 * ✅ variant stock + price uses variant.stocks (safe)
 * ✅ barcode list supports:
 *    - variant.stocks[].barcode
 *    - variant.barcode
 *    - productItem.stocks[] fallback (optional)
 * ✅ Bulk barcode print works from selected rows
 */
export default function Product({ product, filters, unitConversions }) {
  ...
  const formatCurrency = (amount) => {
    ...
    return new Intl.NumberFormat("en-IN", { style: "currency", currency: "BDT" }).format(num);
  };
  ...
  const generateBarcodeImage = (barcode) => {
    ...
    if (!code) return "";
    return `https://barcode.tec-it.com/barcode.ashx?data=${encodeURIComponent(code)}&code=Code128&dpi=96&quiet=0`;
  };
  ...
  // =============================
  // ✅ STOCK CALC (controller loads variants.stocks[])
  // =============================
  const getVariantStocks = (variant) => {
    if (Array.isArray(variant?.stocks) && variant.stocks.length > 0) return variant.stocks;
    // optional fallback if backend sometimes sends single stock
    if (variant?.stock) return [variant.stock];
    return [];
  };

  const getVariantTotalQty = (variant) => {
    const stocks = getVariantStocks(variant);
    return stocks.reduce((sum, s) => sum + (Number(s?.quantity || 0)), 0);
  };

  const getVariantTotalBaseQty = (variant) => {
    const stocks = getVariantStocks(variant);
    return stocks.reduce((sum, s) => sum + (Number(s?.base_quantity ?? s?.quantity ?? 0)), 0);
  };

  const pickVariantSalePrice = (variant) => {
    // strategy: take first stock that has sale_price > 0
    // (you can change to latest stock, max price, etc.)
    const stocks = getVariantStocks(variant);
    const found = stocks.find((s) => Number(s?.sale_price || 0) > 0);
    return Number(found?.sale_price || 0);
  };

  const calculateTotalStock = (productItem) => {
    if (!productItem?.variants || productItem.variants.length === 0) return 0;
    return productItem.variants.reduce((total, variant) => total + getVariantTotalQty(variant), 0);
  };
  ...
  // =============================
  // ✅ Barcode extraction (supports multiple backend shapes)
  // =============================
  const getVariantBarcodes = (variant, productItem = null) => {
    const result = [];

    // 1) variant.stocks[] (multiple) - BEST for your controller
    const stocks = getVariantStocks(variant);
    if (stocks.length > 0) {
      stocks.forEach((s) => {
        if (s?.barcode) {
    ...
    // 2) variant.barcode (single) if present
    if (variant?.barcode) {
      result.push({
    ...
    // 3) productItem.stocks[] fallback (if your product endpoint ever includes it)
    ...
    // unique by barcode+batch (barcode alone sometimes reused in other systems)
    const uniq = [];
    const seen = new Set();
    for (const row of result) {
      const code = String(row?.barcode || "").trim();
      const batch = String(row?.batch_no || "").trim();
      const key = `${code}__${batch}`;
  ...
  const toggleSelectBarcode = (row) => {
    ...
    setSelectedBarcodeMap((prev) => {
      ...
      // use barcode+batch to be safe
      const batch = String(row?.batch_no || "").trim();
      const key = `${code}__${batch}`;
      if (next.has(key)) next.delete(key);
      else next.set(key, { ...row, __key: key });
      return next;
    });
  };
  ...
  const handleBulkBarcodePrint = () => {
    ...
    const { showProductName, showBatchNo, showSalePrice, align, labelWidthMm, labelHeightMm, gapMm, barcodeImgHeightPx } =
      barcodeConfig;
                ...
                return (
                  <>
                    <tr key={productItem.id} className="hover:bg-gray-50">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {(isExpanded || variantsCount <= 1) &&
                            productItem?.variants?.map((variant) => {
                              ...
                              const variantStock = getVariantTotalQty(variant);
                              const variantPrice = pickVariantSalePrice(variant);
                      ...
                      {/* Barcodes column with selection */}
                      <td>
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center gap-2">
                                  ...
                                  <div key={variant.id} className="border rounded-lg p-2 bg-white">
                                    ...
                                    <div className="space-y-1">
                                      {barcodes.map((b, idx) => {
                                        ...
                                        const batch = String(b?.batch_no || "").trim();
                                        const key = `${code}__${batch}`;
                                        const isSelected = selectedBarcodeMap.has(key);
                                          ...
                                          >
                                            <div className="flex items-center gap-2 min-w-0">
                                              ...
                                              <div className="min-w-0">
                                                <div className="font-mono truncate max-w-[160px]">{code}</div>
                                                <div className="text-[10px] text-gray-500">
                                                  Batch: {batch || "N/A"} • Qty: {b.quantity || 0}
                                                </div>
                                              </div>
                                            </div>

                                            <div className="flex gap-1 shrink-0">
                                              <button onClick={() => copyBarcode(code)} className="btn btn-xs btn-ghost" title="Copy Barcode">
                                                <Copy size={10} />
                                              </button>
                                            </div>
                                          </div>
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-1">
                                        {barcodes.map((barcodeData, index) => {
                                          const code = String(barcodeData?.barcode || "").trim();
                                          const batch = String(barcodeData?.batch_no || "").trim();
                                          const key = `${code}__${batch}`;
                                          const isSelected = selectedBarcodeMap.has(key);
                                            ...
                                            >
                                              <div className="flex items-center gap-2">
                                                ...
                                                <div>
                                                  ...
                                                  <div className="text-gray-500">Batch: {batch || "N/A"}</div>
                                                </div>
                                              </div>

                                              <div className="flex gap-1">
                                                <button onClick={() => copyBarcode(code)} className="btn btn-xs btn-ghost">
                                                  <Copy size={10} />
                                                </button>
                                              </div>
                                            </div>
